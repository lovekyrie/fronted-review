...
- 头部增删 `shift/unshift`：`O(n)`（需要搬移元素）。
- 中间插删 `splice`：通常 `O(n)`。
- 随机访问 `arr[i]`：`O(1)`。

##### 7.2 面试高频选型表达
1. 频繁查找存在性：`Set/Map` 优于 `indexOf/includes` 线性扫描。
2. 频繁头部操作：考虑队列结构（双端队列）而非普通数组。
3. 需要不可变更新：优先 `map/filter/reduce` 或扩展运算符。

##### 7.3 易错点清单
1. `sort()` 默认按字符串排序，数字排序要传比较函数。
2. 原地修改和非原地方法混用导致副作用（如 `sort/splice`）。
3. 大数组场景下频繁 `shift` 导致性能退化。