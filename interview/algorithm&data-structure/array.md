...
### 1) 题型识别信号
- 求和组合：`Two Sum / Three Sum`（哈希 + 排序双指针）。
- 原地修改：移动零、合并有序数组（双指针）。
- 连续子数组最值：Kadane / 滑动窗口。

### 2) 通用解题框架
1. 先判定是否需要排序（影响去重和双指针可行性）。
2. 明确是否要求“原地”与“稳定顺序”。
3. 给出指针/哈希策略并写清去重规则。

### 3) 复杂度速答
- Two Sum（哈希）：时间 `O(n)`，空间 `O(n)`。
- Three Sum（排序 + 双指针）：时间 `O(n^2)`，空间约 `O(1)`（不算输出）。

### 4) 高频易错点
1. Three Sum 去重不完整（`i`、`L`、`R` 三处都可能重复）。
2. 合并有序数组从前往后写导致数据覆盖。
3. 忽略空数组或全负数边界（最大子数组和题）。
