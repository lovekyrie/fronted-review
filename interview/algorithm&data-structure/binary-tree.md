...
##### 7.1 DFS vs BFS 如何快速回答
- DFS（前中后序）：适合路径、子树性质、递归定义问题。
- BFS（层序）：适合最短层数、逐层处理、层级视图问题。

##### 7.2 递归写树题的统一模板
1. 函数定义清楚：`fn(node)` 返回什么含义。
2. base case：`node === null` 时返回值。
3. 分解子问题：左子树、右子树。
4. 合并答案：基于左右返回值构造当前结果。

##### 7.3 二叉树常见复杂度
- 遍历时间通常 `O(n)`。
- 递归空间是树高 `O(h)`；平衡树约 `O(log n)`，退化链表是 `O(n)`。

##### 7.4 高频易错点
1. 递归终止条件漏写导致空指针异常。
2. 平衡树判断写成重复计算高度导致 `O(n^2)`。
3. 序列化/反序列化时空节点标记不一致。