...
### 1) 题型识别信号
- 求“最多/最少/最优”且每步决策看起来可局部确定。
- 常见于区间调度、资源分配、跳跃覆盖、找零等问题。

### 2) 标准回答结构
1. 先给贪心策略（例如“按右端点最小优先”）。
2. 再说明为什么局部最优能导向全局最优（交换论证/反证思路）。
3. 最后给排序 + 一次遍历实现。

### 3) 复杂度速答
- 若涉及排序：时间通常 `O(n log n)`。
- 遍历部分一般 `O(n)`，额外空间多为 `O(1)` 或 `O(n)`（视实现）。

### 4) 易错点清单
1. 只给直觉不证明策略正确性，面试容易被追问卡住。
2. 区间题排序关键字选错（有时应按结束点，不是开始点）。
3. 股票 II 误当成只能交易一次，导致写成 DP 或双循环。
