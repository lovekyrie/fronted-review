...
##### 6.1 常用套路速记
1. dummy 节点：统一处理头节点边界。
2. 快慢指针：中点、环检测、倒数第 N 个。
3. 三指针：反转链表（`prev/curr/next`）。

##### 6.2 复杂度速答
- 遍历、查找、反转通常 `O(n)` 时间，`O(1)` 额外空间。
- 链表归并排序：`O(n log n)` 时间，递归栈 `O(log n)`。

##### 6.3 高频易错点
1. 修改 `next` 前未保存后继，导致链断裂。
2. 删除节点时空指针判断遗漏。
3. 快慢指针初始位置不一致导致“中点偏左/偏右”错误。