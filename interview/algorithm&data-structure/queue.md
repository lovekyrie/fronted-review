...
### 1) 题型识别信号
- 层序遍历、滑动窗口、请求时间窗口、先进先出调度。
- 常见结构：普通队列、循环队列、双端队列（单调队列）。

### 2) 通用解题框架
1. 明确队列存放的是值、下标还是节点对象。
2. 维护入队规则与过期出队规则。
3. 在合适时机读取队首作为当前答案。

### 3) 复杂度速答
- 基础队列操作均摊 `O(1)`。
- 单调队列解滑窗最大值总体 `O(n)`（每个元素最多进出一次）。

### 4) 高频易错点
1. JS 里频繁 `shift()` 在大数据下性能一般，可改手写双端队列。
2. 滑窗题“过期下标出队”条件写错（`<= i-k`）。
3. 循环队列满/空判定和指针取模边界处理错误。
