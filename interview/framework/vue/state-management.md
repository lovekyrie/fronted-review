...
##### 6.1 Pinia vs Vuex 面试快答模板
- Pinia API 更轻量，和组合式 API 集成更自然。
- Pinia 默认就是“扁平 store”，不需要 Vuex 那种嵌套 modules 心智负担。
- TypeScript 推导体验通常更好，官方也推荐 Vue3 项目优先 Pinia。

##### 6.2 什么状态该放全局
建议放全局的状态：
1. 多页面共享且频繁读取（用户信息、权限、主题）。
2. 需要跨组件同步（筛选条件、购物车）。
3. 需要持久化恢复（登录态、偏好设置）。

不建议放全局：
- 只在单组件使用的 UI 状态（弹窗开关、输入框草稿）。

##### 6.3 持久化与安全边界
- 持久化一般用 `localStorage` 或 `sessionStorage` 插件。
- 敏感信息（如长期有效 token）不要明文持久化；更推荐 HttpOnly Cookie 方案由后端协同。

##### 6.4 性能优化点
1. 选择器/计算属性只暴露必要派生状态。
2. 避免在 store 中保存过大冗余结构。
3. 大对象更新尽量精细化，减少无关组件响应。