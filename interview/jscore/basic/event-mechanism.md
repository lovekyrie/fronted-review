...
**`closest` 的查找行为**：`closest` 从自身开始向上查找，包含自身。所以两种情况都能正确处理：

| 点击位置 | `e.target` | `closest('[data-id]')` 结果 |
|---------|------------|---------------------------|
| 点 `<span>` 子元素 | `<span>` | 向上找到最近的带 `data-id` 的祖先 `<li>` |
| 点 `<li>` 自身 | `<li>` | 直接返回自身（因为自身就匹配） |

**即：点击哪个子元素都能准确定位到"列表项"这个语义边界，不会报错。**
