function incrementString(strng) {
  ...
  if (match) {
    ...
    return strng.slice(0, strng.length - len) + (Number.parseInt(match[0]) + 1).toString().padStart(len, '0')
  }
  return `${strng}1`
}
...
function incrementedString1(string) {
  if (isNaN(Number.parseInt(string[string.length - 1])))
    return `${string}1`
  return string.replace(/(0*)(\d+)$/, (match, p1, p2) => {
    const incremented = Number.parseInt(p2) + 1
